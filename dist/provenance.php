...
require("auth.php");
    ...
    include_once("process_provenance.php");
    include 'isAdmin.php'; 
...
.resizable-input {
    width: 450px; /* Vous pouvez ajuster la largeur initiale selon vos besoins */
}
                        ...
                        <div class="body">
                            <div class="row clearfix row-deck mt-4">
                                <div class="col-lg-12">
                                    <div class="card">
                                        <div class="header">
                                            <h2>Ajouter un type provenance</h2>
                                        ...
                                        <div class="body">
                                            <form  method="post" action="prove.php">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="type_provenance" class="form-control resizable-input" required>
                                                </div>
                                                ...
                                                <br>
                                                <button type="submit" name="valide" class="btn btn-primary">Validate</button>
                                                <?php
...
$rq1=mysqli_query($conn,"SELECT * FROM `provenance_montant` WHERE `provenance_montant`.`actif` =1");
    ...
    //var_dump($rst);
                                      ...
                                      ?>
                                        <tr>
                                            ...
                                            <a href="edit_provenance.php?edit=<?php echo $rst['id_provenance_montant'];?>"
                                                  class="btn btn-info"><i class="fa-solid fa-pen-to-square"></i></a>

                                            <a href="javascript:void(0);" class="btn btn-danger" onclick="confirmerSuppression(<?php echo $rst['id_provenance_montant'];?>)"><i class="fa-solid fa-trash"></i></a>
...
function confirmerSuppression(id_provenance_montant) {
    ...
    if (confirmation) {
        ...
        window.location.href = "suppression_logique_provenance.php?id_provenance_montant=" + id_provenance_montant;
    } else {
        ...
        document.getElementById('generer-pdf').addEventListener('click', function() {
            // Utilisez AJAX pour appeler le script PHP de génération du PDF
            var xhr = new XMLHttpRequest();
            xhr.open('GET', 'generate_pdf.php', true);
            xhr.responseType = 'blob'; // Spécifiez que la réponse est un blob (fichier)
            
            xhr.onload = function() {
                if (xhr.status === 200) {
                    // Créez un objet URL pour le fichier blob
                    var blob = new Blob([xhr.response], { type: 'application/pdf' });
                    var url = window.URL.createObjectURL(blob);
                    
                    // Créez un lien pour le téléchargement du PDF
                    var a = document.createElement('a');
                    a.href = url;
                    a.download = 'liste_enregistrements.pdf'; // Nom du fichier PDF à télécharger
                    document.body.appendChild(a);
                    a.click(); // Déclenchez le téléchargement
                    window.URL.revokeObjectURL(url); // Libérez l'URL de l'objet blob
                }
            };
